...
Qt部件几乎拥有所有必要的属性，可以开箱即用。很少会遇到必须构建自己的小部件的情况。然而，有些团队为他们的软件创建了一整套定制小部件。如果您计划为程序定制外观，那么这是很好的。例如，可以合并平面样式的小部件，这意味着您必须修改框架提供的默认小部件的样式。自定义小部件应该继承自QWidget(或其任何后代)，如下所示: \par
