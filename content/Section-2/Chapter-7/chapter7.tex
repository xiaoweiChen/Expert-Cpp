...
声明了一个具有重载函数操作符的类。每当重载类的操作符时，就使其可调用。听起来很明显，任何可调用都视为函数。因此，具有重载函数操作符的类对象可以视为函数(也称为函子)。这在某种程度上更像是一个技巧，因为我们没有把函数变成对象，而是把对象变成了可调用的函数。然而，这让我们能够实现我们所寻找的:一个具有状态的函数。下面的客户端代码演示了函数对象有一个状态: \par
